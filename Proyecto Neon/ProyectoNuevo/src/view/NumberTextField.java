...
public class NumberTextField extends Componente implements FocusListener, KeyListener, DocumentListener {

	public AbstractFormatter getFormatter() {
		return getTf().getFormatter();
	}

	public NumberTextField(String initialValue) {
		this(0, 1000d, initialValue);
	}

	public NumberTextField(double max, String initialValue) {
		this(0, max, initialValue);
	}

	public NumberTextField(double max) {
		this(0, max);
	}
	...
	public NumberTextField(double min, double max, String initialValue) {
		this(min, max, new Color(100, 100, 100), initialValue);
	}

	public NumberTextField(double min, double max, Color c) {
		this(min, max, c, "");
	}

	public NumberTextField(double min, double max, Color defaultColor, String initialValue) {
		setTf(new JFormattedTextField(createNumberFormat(min, max)));
	...
	public static MaskFormatter createMaskFormat(String value) {
	...
	public void removeAllTextfieldListeners() {
		...
		for (int i = 0; i < lista.size(); i++)
			getTextfieldListeners().remove(0);
	}

	@Override
	public void insertUpdate(DocumentEvent e) {
		if (isUpdateActivated())
			for (TextfieldListener l : getTextfieldListeners())
