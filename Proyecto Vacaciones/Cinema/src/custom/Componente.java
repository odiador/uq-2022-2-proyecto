...
public class Componente extends JPanel implements MouseListener, MouseMotionListener {
	...
	public static final Color defaultColor = new Color(50, 50, 50);
	...
	public void setBackground (int bg) {
		if (bg == brighter) setBackground(getColor().brighter());
		if (bg == darker) setBackground(getColor().darker());
	...
	public void mouseReleased (MouseEvent e) {
		...
		for (ClicListener b : getListaClicListener())
			if (getMousePosition() != null) b.botonPresionado(new Evento(this));
	...
	public void mouseDragged (MouseEvent e) {
		if (isCanBeArrastred()) setLocation(getX() - (getWidth() / 2) + e.getX(),
				getY() - (getHeight() / 2) + e.getY());
	}
