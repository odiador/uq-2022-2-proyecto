...
public class PanelVerClientes extends Panel {
	...
	public PanelVerClientes(Cliente[] c) {
		...
		for (int i = 0; i < c.length; i++) clientes[i] = c[i];
		...
		for (int i = 0; i < clientes.length; i++) {
			...
			if (tBasic.getExisteTarjeta()) {
				...
				bTarjeta.configAsButton(e -> JOptionPane.showMessageDialog(null, tBasic.toString()));
				add(bTarjeta, gbc);
			} else add(new TableText("-", f, b), gbc);

			gbc.gridx++;
			if (tGold.getExisteTarjeta()) {
				TableText bTarjeta = new TableText("Ver tarjeta Gold", f, b);
				bTarjeta.configAsButton(e -> JOptionPane.showMessageDialog(null, tGold.toString()));
				add(bTarjeta, gbc);
			} else add(new TableText("-", f, b), gbc);
