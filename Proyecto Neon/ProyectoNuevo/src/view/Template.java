...
public abstract class Template extends JFrame implements ClicListener, MouseMotionListener, MouseListener, KeyListener {
	...
	public Template() {
		...
		setPanelSuperior(new Panel(Color.black));
		...
		getCerrar().setFont(Utilities.SUPERIOR_BUTTON_FONT);
		getMinimizar().setFont(Utilities.SUPERIOR_BUTTON_FONT);
		getMaximizar().setFont(Utilities.SUPERIOR_BUTTON_FONT);
		...
		getPanelBody().setBackground(Color.black);
	...
	public abstract void configSize();
	...
	public void botonPresionado(Evento e) {
		if (e.getSource() == getCerrar()) {
			if (getCloseOperation() == WindowConstants.EXIT_ON_CLOSE)
				System.exit(0);
			if (getCloseOperation() == WindowConstants.DISPOSE_ON_CLOSE)
				dispose();
			if (getCloseOperation() == WindowConstants.HIDE_ON_CLOSE)
				setExtendedState(JFrame.ICONIFIED);
		}

		if (e.getSource() == getMinimizar())
			setExtendedState(JFrame.ICONIFIED);
	...
	public void mouseDragged(MouseEvent e) {
		if (e.getSource() == getPanelSuperior()) {
			if (getExtendedState() == JFrame.NORMAL) {
				...
				if (getPanelSuperior().getX() + e.getX() < (getWidth() / 2))
					setLocation(e.getXOnScreen() - e.getX() - getPanelSuperior().getX(), posy);
					...
					setLocation(e.getXOnScreen() - getWidth() / 2, posy);
			}
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		setyArrastre(e.getYOnScreen());
		if (getExtendedState() == JFrame.NORMAL)
			setPosInicial(getLocation());
	}

	@Override
	public void mouseReleased(MouseEvent e) {
		if (e.getYOnScreen() <= 3 && getExtendedState() == JFrame.NORMAL) {
			if (getPosInicial() != null)
				setLocation(getPosInicial());
			actualizarMaximizar();
		}
	}
